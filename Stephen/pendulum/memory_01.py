...
# toolbox.register("expr", generate_safe, pset=pset, min_=2, max_=3, terminal_types=terminal_types)
...
def evalIndividual(individual, test=False):
    env = env_train
    num_episode = 10 # Basically the amount of simulations ran
    if test:
        env = env_test
        num_episode = 3
    ...
    for x in range(0, num_episode):
        ...
        while not (done or timeout):
            ...
            if failed:
                ...
                action = get_action(m, observation[0], observation[1])
                action = (action, )
...
def main():
    ...
    pop, log = algorithms.eaSimple(pop, toolbox, 0.5, 0.5, 50, stats=mstats, halloffame=hof, verbose=True)
